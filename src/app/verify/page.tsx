...
function sampleToFile(sample: (typeof SAMPLE_DOCUMENTS)[number]): File {
  const binary = atob(sample.base64Data);
  const bytes = new Uint8Array(binary.length);
  for (let index = 0; index < binary.length; index += 1) bytes[index] = binary.charCodeAt(index);
  return new File([bytes], sample.fileName, { type: 'application/pdf' });
}
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-5xl flex-col px-6 pb-24 pt-36">
      ...
      <section className="rounded-3xl border border-[#272727] bg-[#181818] p-6 md:p-8">
        <div className="mb-6 flex items-center justify-between gap-4 border-b border-[#272727] pb-5"><div><h2 className="text-xl font-bold text-white">Document check</h2><p className="text-sm text-[#9B9B9B]">Use a demo document or upload your own file.</p></div><label className="flex items-center gap-2 text-xs text-[#9B9B9B]"><span>Test a changed file</span><input type="checkbox" checked={tamper} onChange={(event) => setTamper(event.target.checked)} /></label></div>

        <div className="mb-6">
          <p className="mb-3 text-sm font-semibold text-white">Quick demo documents</p>
          <div className="grid gap-3 md:grid-cols-3">
            {SAMPLE_DOCUMENTS.slice(0, 3).map((sample) => (
              <button key={sample.id} type="button" onClick={() => { setSelectedSample(sample.id); void verifyUpload(sampleToFile(sample)); }} className="rounded-2xl border border-[#313131] bg-[#1F1F1F] p-4 text-left transition-all duration-700 ease-[cubic-bezier(0.32,0.72,0,1)] hover:border-[#2DD4BF] hover:bg-[#272727] focus:outline-none focus:ring-2 focus:ring-[#2DD4BF]">
                <span className="flex items-center gap-2 text-sm font-semibold text-white"><FileText size={18} className="text-[#2DD4BF]" /> {sample.name}</span>
                <span className="mt-2 block text-xs leading-relaxed text-[#9B9B9B]">{sample.expectedOutcome === 'tampered' ? 'Tampered demo' : 'Valid demo'}</span>
              </button>
            ))}
          </div>
        </div>

        <div className="rounded-2xl border-2 border-dashed border-[#313131] bg-[#1F1F1F] p-10 text-center"><input ref={fileInputRef} type="file" className="hidden" onChange={(event) => { const file = event.target.files?.[0]; if (file) void verifyUpload(file); }} /><UploadSimple size={30} className="mx-auto text-[#2DD4BF]" /><h2 className="mt-3 text-xl font-bold text-white">Choose a document</h2><p className="mt-2 text-sm text-[#9B9B9B]">The file is read locally and its fingerprint is calculated in your browser.</p><button type="button" onClick={() => fileInputRef.current?.click()} className="mt-5 rounded-xl bg-[#2DD4BF] px-5 py-3 font-semibold text-black">Browse files</button>{customFile && <p className="mt-4 text-sm text-[#9B9B9B]">Selected: {customFile.name}</p>}{selectedSample && <p className="sr-only">Selected sample: {selectedSample}</p>}</div>
