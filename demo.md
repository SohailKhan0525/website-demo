...
1. Open Claude Code (or v0.dev / bolt.new / Cursor).
2. Paste everything from "AGENT INSTRUCTIONS START" below to the end of
   this file, in one message.
3. When it asks for your Supabase URL and anon key, go create a free
   project at supabase.com, run the SQL in this file, then paste both
   values back to it.
4. When it gives you the deployed Vercel URL, open it on your phone too,
   not just your laptop, before the presentation.
...
- Do not write any placeholder text, "TODO", "example.com", "lorem ipsum",
  sample company names, or dummy copy anywhere. Every piece of text on the
  site is given to you exactly below. Use it exactly. If something is
  genuinely not specified, ask the user in chat instead of inventing it.
- Do not fake, mock, stub, or skip the Supabase integration. It must be a
  real Supabase project with a real table, queried with the real client
  library. The only allowed fallback is the offline fallback described
  below, and even that fallback data must be the same real seed rows, not
  invented ones.
- Before writing any code that touches Supabase, stop and ask the user for
  `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_ANON_KEY`. Do not
  scaffold the rest of the app first and come back to this later, ask for
  these two values up front, in your very first response, before doing
  anything else. If the user does not provide both values, or says they do
  not have a Supabase project yet, stop the entire task and tell them to
  create a free project at supabase.com and run the SQL schema given below,
  then come back. Do not proceed with a fake/local-only version instead.
- This must end as a complete, working, deployed demo, not a partial
  scaffold. Do not stop after generating files. Run it, fix whatever
  breaks, and deploy it, as covered in the Deploy section at the end. Do
  not tell the user to deploy it themselves unless the deploy step itself
  fails and needs their login/credentials.
- Build every page and the full verify flow described below, all of it,
  not a subset. This is for a live hackathon showcase tomorrow, so
  "mostly working" is not acceptable, it needs to fully work end to end:
  upload or pick a sample, real hash computed, real Supabase lookup, real
  result shown, offline fallback actually tested by disconnecting wifi.
- When you believe the build is complete, go through the ship checklist
  near the end of this file item by item and confirm each one, don't just
  assume it passes.
